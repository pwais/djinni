...
object YamlGenerator {
  def metaFromYaml(td: ExternTypeDecl) = MExtern(
    ...
    defType(td),
    td.body,
    toMExtern(td, "cpp", (props: Map[String, Any]) => 
                            MExtern.Cpp(
                              props("typename").toString,
                              props("header").toString,
                              props("byValue").asInstanceOf[Boolean])),
    toMExtern(td, "objc", (props: Map[String, Any]) => 
                            MExtern.Objc(
                              props("typename").toString,
                              props("header").toString,
                              props("boxed").toString,
                              props("pointer").asInstanceOf[Boolean],
                              props("hash").toString)),
    toMExtern(td, "objcpp", (props: Map[String, Any]) =>
                            MExtern.Objcpp(
                              props("translator").toString,
                              props("header").toString)),
    toMExtern(td, "java", (props: Map[String, Any]) =>
                            MExtern.Java(
                              props("typename").toString,
                              props("boxed").toString,
                              props("reference").asInstanceOf[Boolean],
                              props("generic").asInstanceOf[Boolean],
                              props("hash").toString)),
    toMExtern(td, "jni", (props: Map[String, Any]) =>
                            MExtern.Jni(
                              props("translator").toString,
                              props("header").toString,
                              props("typename").toString,
                              props("typeSignature").toString))
  )

  private def toMExtern[T >: Null](td: ExternTypeDecl, key: String, ctor: (Map[String, Any] => T)) : T = {
    td.properties.get(key) match {
      case Some(rawProps: JMap[_, _]) => {
        ctor(rawProps.map(kv => (kv._1.toString, kv._2)).toMap)
      }
      case _ => null
    }
  }
